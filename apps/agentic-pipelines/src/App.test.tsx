...
describe("App", () => {
  ...
  it("renders hero content and workflow picker", () => {
    ...
    expect(screen.getAllByText(/Cloud Release Autopilot/i).length).toBeGreaterThan(0);
  });
  ...
  it("runs the pipeline and completes all steps", async () => {
    render(<App />);
    const executeButton = screen.getAllByRole("button", { name: /execute pipeline/i })[0];
    ...
    expect(screen.getAllByRole("button", { name: /execute pipeline/i })[0]).toBeEnabled();
  });
});
